...
- `git branch --show-current`, `git log master..HEAD --oneline`, `git diff --stat master...HEAD`.
...
- **Secret scan of the diff** (`git diff master...HEAD`): no keys, tokens, or connection strings; no server-only value behind a public prefix (e.g. `NEXT_PUBLIC_*`). Cross-check names against the server-only secrets recorded in `docs/ENV-VARS-SAFETY.md` and PROJECT-STATUS §9; treat any name documented as deliberately-never-set as expecting zero references.

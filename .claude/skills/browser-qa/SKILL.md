---
name: browser-qa
description: Browser verification for (Un)Retire using the globally installed Playwright MCP and Agent Browser CLI. Use whenever work needs real-browser eyes - capturing visual QA evidence for a UI sprint (screenshots at 320/768/1440 plus states on the deployed Preview), testing forms and flows on Preview, responsive checks, console/network debugging, reproducing a reported UI bug, or self-capturing the Preview evidence during /close. Triggers - "take screenshots", "test the preview", "check the form", "visual QA", "capture evidence", "why does this look broken", "does it work on mobile", after any UI change before its PR, and whenever QA-CHECKLIST Part 2 evidence is required.
---

# Browser QA — real-browser verification ((Un)Retire)

...
| D-8 | Does a non-production (test) Supabase project exist? Required for Launch Gate test users | Owner confirms; creates one if not, then creates obviously-fake test users in it | S2.3 | Open |
...
| D-10 | Guest Preview end date (31 Aug 2026) — what happens then? | Owner decides; home banner is date-sensitive | 2026-08-31 | Open |
| D-11 | Connect a **production** Supabase MCP server (`supabase-prod-readonly`)? `docs/SUPABASE-MCP-SAFETY.md` records this repo as **Profile A — no production MCP**. Profile B requires the owner's explicit approval **and** a recorded exception (owner, reason, date, allowed feature groups, data classification, removal condition) before S2.2 may wire it | Lean: connect **test only** (Profile A holds) unless the owner approves Profile B in writing. `supabase-test` (read-write) needs no exception | **S2.2** — blocks the prod server only; the test server can ship without it | Open |
...
| 12 | Medium | Home page banner | Guest Preview banner expires 31 August 2026 (6 days from 2026-08-25) | Open — owner decision D-10 (due 2026-08-31 — ahead of S2.1) |

...
| Next action | 1. **The Codex stage-gate review returned STAGE NOT APPROVED (2026-08-25, 2 Blocking findings — both correct).** Finding 1: the gate was run out of order. The chain is branch → checks → PR → tested Preview → **per-PR review → APPROVE → owner merges** → *then* the stage gate runs against the merged `master` range. So: run the **per-PR** review first over the immutable merge-base→head range at the Preview-tested head and save it at `docs/code-reviews/[SPRINT_ID]-[SLUG]-review.md`; re-run the stage gate only after the owner has merged. 2. **D-12 needs the owner's explicit yes/no in writing** (Blocking finding 2: the committed range adds all 57 `Website-Development-System/**` files while D-12 is Open). **The S1 PR cannot merge until they answer.** 3. Deploy and test the Preview for [PR_NUMBER] at [HEAD_SHA]; record [PREVIEW_URL] in §6. 4. **Environment isolation is now a hard prerequisite for any testing.** Preview shares all three Supabase variables with Production, so every Preview deployment reads and **writes** `unretire-prod` (Known issue 17), and Preview has no test-mode Stripe at all (Known issue 18). S2.2 must repoint Preview at `unretire-test` and add Preview-scoped Stripe test-mode values **before** S2.3 or S5.1 run anything — a suite run against production data is not a test. Governing doc: `docs/ENVIRONMENT-PARITY.md`. 5. Owner actions, in order: `docs/OWNER-ACTIONS.md`. — *Prior:* owner review of R1 → R2 Code Check CI (superseded 2026-08-25 by the 5-stage plan) |
| Preview / Production | Vercel Preview deployment (per-PR) / production domain TBC (Open decision D-2) on Vercel. ⚠ Preview and Production currently share Supabase credentials — **not isolated** (Known issue 17) |
| Last updated | 2026-08-25 — Codex stage gate returned **STAGE NOT APPROVED** (2 Blocking): the gate ran before the per-PR review and before merge, and the range carries the SOP folder while D-12 is Open. Environment audit completed against the live Vercel dashboard: D-11 now fully specified (prod ref recorded), D-13 added, Known issues 17–21 added, §9 corrected. — *Prior:* 2026-08-25 — trackers re-pointed from R1–R7 to the owner-approved 5-stage plan (S1–S5); predevelopment withdrawn (D-6); D-11 and D-12 added |
...
| S2.2 — Environment isolation + agent tooling | Not Started | | | | **Leads with environment isolation** (Known issues 17–18): repoint the three Supabase variables so Preview uses `unretire-test`, add Preview-scoped Stripe **test-mode** values, prove no Preview path can reach production data. **Then** the MCP wiring: project-scoped `.mcp.json` with `supabase-test` (read-write; database, debugging, docs) and `supabase-prod-readonly` (`read_only=true`), governed by `docs/SUPABASE-MCP-SAFETY.md` — repo records **Profile A**, so the prod server needs **D-11** (now fully specified). Playwright MCP stays at USER scope (`docs/BROWSER-TOOLS.md`) — verify only. Verify all 5 skills load. Governing doc: `docs/ENVIRONMENT-PARITY.md` |
...
|---|---|---|---|
| NEXT_PUBLIC_SITE_URL | Public | Canonical site URL / `metadataBase` (authoritative for D-2) | ⚠ **NOT SET in any environment** — D-13, Known issue 19 |

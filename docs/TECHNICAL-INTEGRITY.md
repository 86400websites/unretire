...
> ~~but is NOT yet a required status on `master` (owner confirmation pending).~~ **and, since 2026-08-27, it IS a required status on `master`.** **Verified 2026-08-27 from the GitHub API, not from a screenshot:** `GET /repos/86400websites/unretire/rules/branches/master` returns `required_status_checks` with `checks: ['code-check']` and `strict_required_status_checks_policy: false`, alongside `pull_request`, `deletion` and `non_fast_forward`. The check also ran and concluded **success** on PR #12's head `5a567a5`. **Red can no longer reach `master`.** Wall 2 is therefore **fully live**: its present-tense sentences below now read normally.** Every present-tense statement below **about Wall 2 (the Code
...
> and after S2.3 / S5.1 land (Wall 4)**, not today.~~ **Wall 2 is now fully live: S2.1 merged on 2026-08-27 and the
> owner enabled the required `code-check` status the same day (verified from the GitHub API), so every Wall 2
> sentence below — including "red cannot merge" — reads normally. Only Wall 4 (S2.3 / S5.1) remains target state.** *(Walls 1 and 3 are live now and their statements read
...
> | Wall 4 (behavior proof) | ~~**Not live** — Playwright harness is **S2.3**, the suite is **S5.1**; no automated suite exists today *(unchanged 2026-08-27)*~~ **Harness landed 2026-08-28 (S2.3, in progress)** — `@playwright/test`, one `@smoke` spec + three auth setups, run against a deployed Preview by `.github/workflows/e2e-preview.yml` (not a required status); the suite that makes this wall hold is **S5.1** |
>
...
> watched-lock verification is an owner action, pending.~~ **Superseded the same day — the required status is enabled and `code-check` passed on PR #12; red cannot reach `master`.**
...
- [x] **You (2 minutes, once):** ✅ **Done 2026-08-27 —** **the required `code-check` status was enabled by the owner on 2026-08-27 (verified from the GitHub API) and passed on PR #12, so red cannot reach `master`**. ~~GitHub → the repo → **Settings → Branches → Add branch protection rule** → branch name `master` → tick **"Require a pull request before merging"** and **"Require status checks to pass before merging"** → search and select **"Code Check"** → save.~~ ~~**Corrected 2026-08-27 (owner screenshot):** this repo protects `master` with a **Ruleset**, so the path is **Settings → Rules → Rulesets → "Protect master" → Branch rules** → tick **"Require status checks to pass"** → **+ Add checks** → select **"Code Check"** → **Save changes**. "Require a pull request before merging" is already ticked there; leave "Require branches to be up to date before merging" unticked (not in this spec).~~ **All of that was completed by the owner on 2026-08-27; the instruction is kept struck as history.** *(Retrofit note 2026-08-26: the "Require a pull request" half is already enabled on this repo — owner-confirmed; ~~what S2.1 adds is the required "Code Check" status, then the watch-one-PR verification below.~~ both halves were completed on 2026-08-27 — the required `code-check` status is enabled and was watched passing on PR #12.)* *(2026-08-27: still pending — owner action after the S2.1 PR is open, via the GitHub web UI; `gh` CLI not installed.)*
- [x] **You + Claude Code (verify once):** ✅ **2026-08-27** — the required status is enabled and `code-check` concluded **success** on PR #12 (`5a567a5`), which is the PR that then merged; the merge path was gated by it. *(Original text:)* watch one PR — the "Code Check" appears and goes ✅, and GitHub's merge button stays locked until it does. An unverified gate is the same as no gate. ~~*(2026-08-27: pending — until this is recorded, a red PR can still be merged by hand; the workflow runs on every PR from S2.1's merge, but nothing requires it yet.)*~~ ***(Superseded the same day: the owner enabled the required `code-check` status on the `master` ruleset — verified from the GitHub API — and it passed on PR #12. Red cannot reach `master`.)***

## Day to day

*(From S2.1's merge onward —* ⚠ *~~not yet true today; see the status banner.~~ 2026-08-27: the ✅/❌ appears on every PR from S2.1's merge; ~~the "merge button stays locked" half is true only once the owner adds the required status and watches it on a real PR~~ — the required status was enabled 2026-08-27, so both halves are true; see the status banner.)* Every PR now carries a plain ✅ or ❌ before your merge button. On ✅, merge as usual. On ❌, you do nothing — Claude Code reads the failure, fixes it, pushes, and the check re-runs; red can't reach `master`, so there is nothing to worry about, only something to wait for. You never have to wonder whether the checks were *really* run — the merge button is the proof. ~~Until S2.1 lands, the local checks are run and reported by hand in each sprint's record, and the independent review (Wall 3) is the gate that actually holds.~~ **S2.1 landed on 2026-08-27 and the check became a Required status the same day, so the Code Check is now the gate that holds mechanically; the independent review remains the gate for everything a machine cannot judge.**** *(2026-08-27: S2.1 merged as PR #11 (`a68f210`) — each PR now carries the green check, but red can still be merged by hand until the required status is added and watched.)*

## The boundary, one line

The Code Check proves the **code** is sound on every PR; the Launch Gate and morning check prove the **site** behaves. Both, always — neither replaces the other. ~~⚠ **Both are target state**: the Code Check arrives in **S2.1**,~~ the Launch Gate harness in **S2.3** and its suite in **S5.1** are still target state. ~~Today neither is live~~ 2026-08-27: the Code Check is installed on `master` since the S2.1 merge (PR #11, 2026-08-27) (`a68f210`) and runs on every PR from its merge ~~but is not yet a required status~~ **and has been a required status since 2026-08-27**; the Launch Gate is still not live — the independent review (Wall 3) plus hand-run local checks carry the load.

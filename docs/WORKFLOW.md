...
Every change to `(Un)Retire` follows:
...
> added in S2.1.~~
>
> ⚠ **Status, 2026-08-27 (Sprint S2.1, in progress — merged 2026-08-27 as PR #11 (`a68f210`)):** the **Code Check (CI)** workflow is installed by Sprint
...
- [ ] Sync from the latest `master` without discarding user work.
...
**Action boundary:** do not commit or push unless the owner explicitly authorized that action. If not authorized, leave the verified diff uncommitted and report a suggested commit message. Never push to `master`, force-push shared work, reset user work, or skip hooks.

## 4. Pull Request

- [ ] The owner-authorized branch is pushed and a focused PR targets `master`.

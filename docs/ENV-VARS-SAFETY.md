...
The rules for handling configuration values and secrets on (Un)Retire. Read this once before touching any key; follow the procedures every time after.
...
|---|---|---|---|
| Site URL | `NEXT_PUBLIC_SITE_URL` | Public | Local + approved host environments |
...
| Email provider key *(example)* | `MAILCHIMP_API_KEY` | Private | Trusted local/server secret stores |
| Database secret key *(optional)* | `SUPABASE_SECRET_KEY` | Private | Trusted server secret store; avoid unless required |
| Bot-protection secret *(example)* | N/A — none wired yet (required before launch, Sprint S4.5) | Private | Trusted local/server secret stores |

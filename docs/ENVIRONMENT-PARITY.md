...
by the owner on `unretire-test` the same day — **but three production cells are unread (Providers → Email detail;
leaked-password protection; captcha), and this proof requires a complete table evidenced from both dashboards, so
it stays PARTIAL until they arrive.** Note the plan tiers:

...
> from the owner's reads as they arrive: **P3** (the reset e-mail was sent — auth log `mail.send recovery` — the
> landing origin is the owner's read), **P4** (Sandbox dashboard), **P12** (re-affirmation), **P13**'s email half
> (a Production reset the owner requests). **P1, P11, P12** stand from S2.3/S2.2. **P7 N/A** (D-22). Until the owner's
> reads are filed the rows below say exactly which half is in hand. *(What follows is the earlier history of this
> status, retained.)*
>

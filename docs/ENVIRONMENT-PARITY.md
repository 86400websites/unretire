...
here. **P9 status (corrected 2026-08-29 after Round 1 Finding 1): PARTIAL, not PASS.** Every row read matches or is
by design — the one mismatch found on the first read ("Confirm email" ON in test, OFF in production) was corrected
by the owner on `unretire-test` the same day — **but the table must be complete before this proof can be PASS.
Narrowed 2026-08-29: production's Providers → Email detail was read and is identical to test (leaked-password
protection OFF on both), so ONE cell is still unread — production's Attack Protection → Captcha. P9 stays PARTIAL
until it arrives.** Note the plan tiers:
...
| Captcha protection | *not yet read — the only cell still owed* | OFF | **owed (prod)** | Material if it differs: with captcha ON in production and OFF in test, a Preview signup would pass while a real one is rejected for a missing token (`/api/*` sends none) — the §7 risk-2 shape. The app has no captcha integration at all (abuse controls are S4.5, D-9), and production signups demonstrably work, but that is inference: **P9 stays PARTIAL until the owner reads `unretire-prod` → Authentication → Attack Protection** |
...
> catalog diff, empty), **P9 PARTIAL** (§5.3c — one mismatch found and corrected the same day; three production cells unread, so not PASS — Round 1 Finding 1), **P10 PASS**. Recorded
...
| **P9** | **The auth-settings parity table is filled and evidenced** — 🟡 **PARTIAL — NOT PASS (corrected 2026-08-29, Round 1 Finding 1).** This proof's own condition is a *complete* table evidenced from **both** dashboards, and three production cells are unread (below), so it may not be recorded PASS until they arrive. What is recorded, and stands: **every row read matches or is by design**, §5.3c — every row read from both dashboards matches or is by design; the one mismatch the first read found — "Confirm email" ON in test, OFF in production — was corrected on `unretire-test` by the owner the same day and cross-checked by the signup spec's observed immediate-login branch. **Owed before P9 can be PASS (narrowed 2026-08-29):** ~~production's **Providers → Email** detail (minimum password length, secure email change, OTP length/expiry) and~~ production's Providers → Email detail was read the same day and is **identical to test** (§5.3c), and the leaked-password row came with it (**OFF on both**). **One cell remains: production's Attack Protection → Captcha.** The builder's earlier argument that these "cannot fork a behaviour the harness exercises" is an inference, not evidence, and does not substitute for the reading — Round 1 Finding 1. Once supplied, the three cells are filled, every row compared, and any mismatch resolved or justified before P9 is recorded PASS | Walk §5.3's table in both dashboards and record each pair. | Every row matches, or a difference is recorded with an explicit reason and a compensating check. | Unknown behaviour differences — §7 risk 2. |

...
| **Site URL — actual, 2026-08-25** | ✅ **`https://unretire.vercel.app`** — correct for today | ✅ **`http://localhost:3000`**, with `localhost:3000/**` and `*-86400-s-projects.vercel.app/**` allow-listed — correct for a test project whose only clients are local dev and Preview |
...
| 9 | ~~**Configure Site URL + the redirect allow-list in BOTH Supabase projects** (§5.3a) — the production project is currently sending every auth email link to `http://localhost:3000`.~~ | **owner** | ✅ **DONE 2026-08-25 — both projects configured; only the end-to-end proof (P3) remains, owed by S2.5.** ~~Previously logged 🟡 HALF DONE 2026-08-25.** `unretire-prod` configured — Site URL `https://unretire.vercel.app` plus a five-entry allow-list; the live production defect is fixed and the two legacy hosts are gone (Known issue 28 resolved). **`unretire-test` is still to do**, so §8 **P3** cannot pass. ⚠ Known issue 23's *configuration* half is closed; its embedded claim that the app's `next` handling is "not an open redirect" is **wrong and retracted** — see §5.3b and Known issue 38 |

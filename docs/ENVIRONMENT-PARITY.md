...
> catalog diff, empty), **P9 PASS** (§5.3c — one mismatch found and corrected the same day), **P10 PASS**. Recorded
...
| **P9** | **The auth-settings parity table is filled and evidenced** — **✅ PASS on every row read, 2026-08-28 (Sprint S2.5), §5.3c — with three production cells still owed:** every row read from both dashboards matches or is by design; the one mismatch the first read found — "Confirm email" ON in test, OFF in production — was corrected on `unretire-test` by the owner the same day and cross-checked by the signup spec's observed immediate-login branch. **Owed:** production's Providers → Email detail (minimum password length, secure email change, OTP), leaked-password protection and captcha — none forks a behaviour the harness exercises (the app enforces its own ≥ 8-character minimum; captcha/leaked-password are S4.5 territory); the owner supplies them and the three cells are filled | Walk §5.3's table in both dashboards and record each pair. | Every row matches, or a difference is recorded with an explicit reason and a compensating check. | Unknown behaviour differences — §7 risk 2. |

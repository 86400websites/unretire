...
| **P9** | **The auth-settings parity table is filled and evidenced** — **✅ PASS 2026-08-28 (Sprint S2.5), §5.3c:** every row read from both dashboards matches or is by design; the one mismatch the first read found — "Confirm email" ON in test, OFF in production — was corrected on `unretire-test` by the owner the same day and cross-checked by the signup spec's observed immediate-login branch; two production rows (Providers → Email detail; Attack Protection) remain to be read but cannot fork behaviour the harness exercises | Walk §5.3's table in both dashboards and record each pair. | Every row matches, or a difference is recorded with an explicit reason and a compensating check. | Unknown behaviour differences — §7 risk 2. |

...
| Gap | Was | Now |
|---|---|---|
| **1** — Preview reads *and writes* production | 🔴 CRITICAL | ✅ **Configured 2026-08-25** — Preview points at `unretire-test`. ~~Unproven until §8 P1 + P2~~ **P1 PASS 2026-08-28 (S2.3); P2 owed by S2.5** |
...
- [ ] **Step 3 — Redeploy the Preview and prove the switch landed.** Run the §8 Proof 1 check. Do not proceed
      ...
      ✅ **DONE 2026-08-28 (S2.5): P1 (S2.3) and P2 (S2.5, run 33197903287) recorded — a running Preview demonstrably reads the test project.** *(Was:)* ~~🔶 **STILL OPEN — this is residual 2.**~~ The variables are in place; nothing has confirmed a running
...
- [ ] **Step 11 — Run every proof in §8 and record the results.** No test suite runs before §8 is green.
      ...
      item in this document. Sprint **S2.5**.~~ **Run in S2.5 (2026-08-28, PR #18): P2, P5, P6, P8, P9, P10 PASS by the
      builder's reads; P1 (S2.3), P11, P12 (S2.2) stand; P3, P4, P13's e-mail half and P12's re-affirmation are the
      owner's reads (OWNER-ACTIONS 6.6–6.9); P7 N/A. See the §8 status note.**
...
**Closed 2026-08-28 (Sprint S2.5):** the definition was captured read-only from `unretire-prod` through the D-11 MCP
and committed as `supabase/migrations/0001_entitlements.sql` (change record `docs/database-changes/0001-entitlements.md`);
`unretire-test` was built from that file, and proof **P8** diffed the two projects empty. Two facts the capture
corrected in the table below: `status` admits **three** values, and there is **no** index on `stripe_subscription_id`.
~~`docs/TECH-ARCHITECTURE.md` §4 already flags this as unverified.~~ Sprint **S4.3** keeps the standing migration workflow
and the index / `search_path` findings; ~~Sprint **S2.5** owns proving parity between `unretire-test` and `unretire-prod` before any suite runs — whichever lands first must not assume the other has.~~ S2.5 delivered both the capture and the parity proof.
...
- [x] **Capture the real definition read-only** — via the approved Supabase MCP Profile B connection
      (D-11: `read_only=true`, feature groups `database,debugging,docs`, per `docs/SUPABASE-MCP-SAFETY.md`),
      or a schema-only dump, or the dashboard's table and policy views. *(Done 2026-08-28, S2.5 — seven
      `pg_catalog`/`information_schema` queries through `supabase-prod-readonly`, listed in the sprint record.)*
- [x] **Commit the captured DDL** into the repo (alongside the existing `book_downloads.sql`, or in a
      `supabase/` migrations folder) so it is version-controlled and reviewable. *(Done 2026-08-28 —
      `supabase/migrations/0001_entitlements.sql` + `.down.sql`; `0002_book_downloads.sql` is the verified copy of
      the existing file.)*
- [x] **Build `unretire-test` from that committed file** — never from a fresh hand-written guess. *(Done
      2026-08-28 — applied through the `supabase-test` MCP under the owner's authorisation; P8 diff empty.)*
...
| `status` values `active` and **`canceled`** (American spelling) — **captured 2026-08-28: production's check admits a THIRD value, `expired`, which the code never writes; kept verbatim in `0001`** | A constraint spelled `cancelled` would break cancellation |
| Stripe customer id and subscription id, both **nullable** — *captured: both nullable; the course row written by the S2.5 sandbox purchase carries nulls (payment mode), the premium row carries both* | The webhook explicitly writes null when Stripe omits them |
| An index on the subscription id — **captured 2026-08-28: production has NO such index** (only the primary key and the `(user_id, product)` unique index); recorded, not added — S4.3 decides | It is the lookup key when a subscription is cancelled |
...
- [x] Diff production against the committed file, then apply the **verified** file to the test project. *(Done
      2026-08-28, S2.5 — no drift; the verified copy is `supabase/migrations/0002_book_downloads.sql`, applied to
      `unretire-test`; change record `docs/database-changes/0002-book-downloads.md`.)*
- [x] Confirm all of: the unique constraint on (user, document type) — this is what actually enforces
      "one download, once"; the document-type check constraint; RLS enabled; exactly two policies (read and
      insert, both own-row-only); and deliberately **no** update or delete policy. Preserve that absence. *(All
      confirmed in production on 2026-08-28 and reproduced identically on test — P8.)*
...
here. **P9 status: ✅ COMPLETE and PASS, 2026-08-29.** The table is now filled from **both** dashboards with no cell
unread — the last three (production's Providers → Email detail, leaked-password protection, and Attack Protection →
Captcha) arrived on 2026-08-29 after Round 1 Finding 1, and **every one matches test**. The single mismatch this
exercise found — "Confirm email" ON in test, OFF in production — was corrected by the owner on `unretire-test` the
same day and cross-checked against the signup spec's observed immediate-login branch. *(History: recorded PASS on
2026-08-28 with three cells unread, demoted to PARTIAL on 2026-08-29 by Round 1 Finding 1 — the demotion was right,
and it is now PASS on evidence rather than on inference.)* Note the plan tiers:
...
| Providers → Email detail | **Read 2026-08-29 (owner screenshot):** enable email provider ON · secure email change ON · secure password change OFF · require current password when updating OFF · **minimum password length 6** · password requirements none · e-mail OTP expiry 3600 s · e-mail OTP length 8 | Enable email provider ON · secure email change ON · secure password change OFF · require current password when updating OFF · **minimum password length 6** · password requirements none · e-mail OTP expiry 3600 s · e-mail OTP length 8 | **Y — identical** | The app enforces ≥ 8 characters itself (`src/app/auth/actions.ts:101`), so the 6-character dashboard minimum is not what gates a signup on either project; the fixture password is > 8 (owner-confirmed 2026-08-28). *Which project the screenshot shows:* the Pro-only qualifier that the test project's identical panel carries on the leaked-password row is **absent** here, which is the tell for the Pro project `unretire-prod` |
| Leaked-password protection | **OFF** (read 2026-08-29 from the same panel — a toggle, not the Pro-gated badge the free project shows) | DISABLED (Pro-only on the free tier — effectively off) | **Y — both off** | Neither project rejects a known-breached password today. Not a parity risk; it is a Production hardening choice for **S4.5** |
| Captcha protection | **OFF** — read 2026-08-29 from `unretire-prod` → Authentication → **Attack Protection** (the screen header names the project and the `PRODUCTION` branch, so there is no ambiguity about which project was read) | OFF | **Y** | Had production been ON with test OFF, a Preview signup would pass while a real one was rejected for a missing token — the §7 risk-2 shape. Both are off; abuse controls arrive in S4.5 (D-9) |
...
> validates the fixture password, one `email` identity each). ~~They carry **no entitlement rows yet**~~ **Since 2026-08-28 (S2.5, run 33197903287) fixtures 3 and 4 carry `active` `entitlements` rows written by the Sandbox webhook from real test-mode purchases — the course row's Stripe ids null by construction (payment mode), the premium row's both set — so "at least one through a real test-mode purchase" is met twice.** *(Historical, 2026-08-28 morning:)* They carried no entitlement rows — `public` is
...
| **C3** | **Preview sits behind Vercel's deployment password; Production does not** | Stripe's webhooks cannot send custom headers, so a protected Preview answers Stripe with a login page. | Enable **Protection Bypass for Automation** (`docs/testing-setup/SETUP-CHECKLIST.md` Part 3) and append the bypass as a **query parameter** on the webhook URL — verify in the dashboard, since a header-only bypass would force the local-CLI route instead. Prove it with an unauthenticated request that succeeds. Separately assert Production has **no** protection, so the open request path is covered too. **Status 2026-08-25:** protection remains **on** and the bypass secret **is provisioned** in the Preview scope; still unconfirmed are (a) ~~whether the Playwright config presents it (S2.3)~~ **~~the config presents it by name (S2.3, 2026-08-28)~~ the shared fixture `tests/e2e/fixtures.ts` attaches `x-vercel-protection-bypass` + `x-vercel-set-bypass-cookie` by name **only to requests whose origin equals the validated deployment under test — never context-wide** (Round-1 remediation; proven by the Round-2 runs 33162485808 (`pull_request`) / 33162520875 (`deployment_status`) at `5ce24cd`), and its end-to-end effect is PROVEN — the `E2E — Preview` runs at PR #16 head `8111884` and again at `5ce24cd` reached the protected Preview (which answers 302 to Vercel SSO without the bypass) and passed** and (b) ~~whether the sandbox webhook URL carries it as a query parameter~~ **(b) VERIFIED 2026-08-28 (S2.5, proof P6): the Sandbox webhook's query-parameter bypass works end to end — Stripe's delivery wrote the `entitlements` rows in `unretire-test`; without the parameter the endpoint answers 401**. Known issue 25, reworded — it no longer blocks all automated testing. |
...
> **Recording status — 2026-08-28, Sprint S2.5 (PR #18), supersedes the paragraph below it.** The parity dispatch
> `E2E — Preview` run [33197903287](https://github.com/86400websites/unretire/actions/runs/33197903287)
> (`workflow_dispatch`, `parity: on`, head `d1b7d2f`) passed **10/10** on the deployed Preview
> `https://unretire-k2rwxafh7-86400-s-projects.vercel.app`, and the builder's read-only reads recorded, with dates:
> **P2 PASS**, **P5 PASS** (the `entitlements` rows themselves, read from `unretire-test`), **P6 PASS** (proven by
> those rows — only the Sandbox webhook writes them — plus the 401 negative control), **P8 PASS** (the prod-vs-test
> catalog diff, empty), **P9 PASS** (§5.3c — complete from both dashboards 2026-08-29; the one mismatch found was corrected the same day), **P10 PASS**. Recorded
> from the owner's reads later the same day: **P3 PASS** (the reset link's `redirect_to` is the Preview deployment),
...
|---|---|---|---|---|
| **P1** | **The Preview deployment resolves to the test project** — **✅ PASS 2026-08-28 (Sprint S2.3):** at PR #16 head `8111884` — `E2E — Preview` runs [33157431610](https://github.com/86400websites/unretire/actions/runs/33157431610) (`pull_request`) and [33157354972](https://github.com/86400websites/unretire/actions/runs/33157354972) (`deployment_status`), 2026-08-28 — every step green including "Run the e2e suite against the deployed Preview"; the three auth-setup projects signed in fixtures that exist only in `unretire-test` (negative control the same day: **0** matches in `unretire-prod`, §5.5) and landed on `/account`. The Preview (`https://unretire-git-claude-s23-playwright-harness-86400-s-projects.vercel.app`, protection ON — it answers 302 to Vercel SSO without the bypass) can therefore only be resolving to `dtdadtggahjsrmevwvbu`. *(2026-08-27: attempted in S2.2 and not observable — ~~the value is inlined in the client JS behind Deployment Protection~~ **corrected 2026-08-28 (S2.3): the ref is in NO client chunk (`grep -r supabase.co .next/static` → nothing) and the browser never calls Supabase, so no request capture is possible**; **deferred to S2.3**, ~~whose harness sends the bypass header~~ whose auth-setup projects carry the sanctioned bypass by name and sign in a fixture that exists only in `unretire-test` — D-20 amended and corrected)* | ~~On the deployed Preview `[PREVIEW_URL]`, inspect the served page for the Supabase project reference (it is a public value and appears in the deployment's own configuration listing). Compare against both refs.~~ **(S2.3, 2026-08-28)** Run the three auth-setup projects against the Preview (`E2E — Preview`): each signs in a fixture account that exists only in `unretire-test` and must land on `/account` showing that fixture's email. Negative control: the fixture emails are absent from `unretire-prod` (read-only count via `supabase-prod-readonly`, or the owner's dashboard). | The reference is **`dtdadtggahjsrmevwvbu`** (`unretire-test`). | It still shows `hcjivvlwxltyiycfbttc` — Phase A did not take effect, most likely because the Preview was not redeployed. Stop. |
| **P2** | **A test signup lands in TEST and is absent from PROD** — **✅ PASS 2026-08-28 (Sprint S2.5):** the parity spec `tests/e2e/parity/signup.spec.ts` signed up an owner-owned plus-tag address on the Preview (runs 33196333939 and [33197903287](https://github.com/86400websites/unretire/actions/runs/33197903287)); read-only afterwards: the account existed in `unretire-test` (confirmed, `last_sign_in_at` set — the immediate-login branch, i.e. "Confirm email" OFF, matching production), and `unretire-prod` held **0** matches for that address and for the three fixtures; the auth log shows `POST /signup` 200 on the test project. The throwaway accounts were deleted from `unretire-test` after the reads (one per dispatch, ledger in the sprint record) | Sign up on `[PREVIEW_URL]` with an obviously-fake address. Then look in **both** Supabase dashboards. | The new user exists in `unretire-test` and **does not exist** in `unretire-prod`. | Preview is still writing to production. Stop everything and re-check P1. This is the definitive test — `docs/SUPABASE-VERCEL-SETUP.md` B6 already names it as the wiring verification. |
| **P3** | **Preview auth emails return to the Preview** — **✅ PASS 2026-08-28 (Sprint S2.5, owner read):** the delivered reset e-mail's link is `https://dtdadtggahjsrmevwvbu.supabase.co/auth/v1/verify?token=…&type=recovery&redirect_to=https://unretire-ani2irxnx-86400-s-projects.vercel.app/auth/confirm?next=/unretire/reset-password` (token redacted; the owner pasted the link's `redirect_to`) — the **Preview deployment** of PR #18's first head, not `www.unretireproject.com`, not `localhost`. *(Mechanism as recorded before the read:)* `password-reset.spec.ts` requested a reset for the signed-in fixture on the Preview; the test project's auth log shows `POST /recover` 200 and **`mail.send` `mail_type: recovery`** (twice — runs 1 and 2 — not rate-limited) with `redirect_to` = the Preview deployment's `/auth/confirm?next=/unretire/reset-password`. Under the default `{{ .ConfirmationURL }}` template the link's own host is `<ref>.supabase.co`; the proof is the origin it **resolves to** — the owner clicks it and reports the address bar (OWNER-ACTIONS 6.6 a). PASS is recorded when that read arrives | From the P2 signup (or a password reset), ~~inspect the link in the email~~ click the link in the e-mail and read the origin in the address bar of the page it lands on (a 404 on `/unretire/reset-password` is expected — Known issue 2), or read the host inside the link's `redirect_to` parameter — never the link's own host. | ~~The link's host is the Preview origin~~ The origin the link resolves to is the Preview origin — **not** the production origin and **not** `localhost`. | ~~The `unretire-test` redirect allow-list is missing the Vercel preview wildcard~~ · ~~**This proof fails today by construction**~~ — **superseded 2026-08-25: both projects are configured.** `unretire-test` has Site URL `http://localhost:3000`, allow-list `http://localhost:3000/**` and `https://*-86400-s-projects.vercel.app/**`, so the Preview wildcard **is** present and this proof is no longer expected to fail for a configuration reason. It is simply **unrun** — no Preview signup has yet been observed returning its link to the Preview origin. Owed by **S2.5**. |
| **P4** | **A Preview payment appears only in Stripe TEST mode** — **✅ PASS 2026-08-28 (Sprint S2.5, owner read):** the Sandbox dashboard (banner "You're testing in a sandbox") lists the two payments — **$199.00 USD Succeeded, Subscription creation, Visa •••• 4242, `thefalafeltheory+ur-e2e-premium@gmail.com`** and **$99.00 USD Succeeded, `thefalafeltheory+ur-e2e-course@gmail.com`**, both Aug 28 6:09 PM (local); the live account's "nothing new" is the owner's standing D-11/§6 C1 read (the live account holds no `+ur-e2e-` customer — nothing in this sprint can reach it: Preview holds only Sandbox keys, P12). *(As recorded before the read:)* `checkout-course.spec.ts` and `checkout-premium.spec.ts` completed one `payment`-mode and one `subscription`-mode checkout on Stripe's hosted page with the public test card from the Preview (run 33197903287); the resulting `entitlements` rows in `unretire-test` (P5) prove the Sandbox account processed them. The "nothing in live mode" half is the owner's Sandbox/live dashboard read (OWNER-ACTIONS 6.7 a) | Complete a checkout on `[PREVIEW_URL]` with the standard test card. Check the Stripe dashboard in **both** modes. | The payment appears under **test mode** and **nothing** appears in live mode. | A live key reached Preview. Stop immediately — this is a real-money leak, and `docs/ENV-VARS-SAFETY.md`'s leak procedure applies (rotate first). |
| **P5** | **The payment actually granted access — in the test database** ⚠ **the single most important proof in this table** — **✅ PASS 2026-08-28 (Sprint S2.5, run [33197903287](https://github.com/86400websites/unretire/actions/runs/33197903287)):** read from `unretire-test` after the run — `course` fixture: `product = course`, `status = active`, Stripe ids **null** (payment mode — the documented shape, "the webhook explicitly writes null when Stripe omits them"); `premium` fixture: `product = premium`, `status = active`, `stripe_customer_id` and `stripe_subscription_id` **set**; both written 18:09:11–13Z by the Sandbox webhook on the `staging` deployment (the table's only writer), and both fixtures' `/account` rendered "Go to the course →" with the badge in the same run. Not inferred from the redirect, the banner or a 200. `unretire-prod`: 0 rows for any fixture; total unchanged (15) | *(2026-08-27: Known issue 32 cleared - `staging` is built and its alias answers, so P5 is runnable but unrun, owned by S2.5.)* After P4, check `unretire-test` for the access record, and load the gated content as that fixture user. **Read the database. Do not accept the redirect, the HTTP 200, Stripe's "Delivered", or the "Payment successful" banner** — with Known issues 22 and 45 open, all four are printed regardless of whether the write succeeded (§7 callout). | The record exists with status `active`, **and** the gated page opens for that user. | The webhook did not deliver, or the write failed and was silently swallowed. Check the Stripe endpoint's delivery log; remember the "invalid signature" message can actually mean a missing key (§3 Gap 2 / Known issue 36), and that a database write error is currently answered with a success code (§7 risk 1, Known issue 22) — so a green delivery log does **not** prove the write. Today, delivery is additionally impossible ~~until Known issue 32 is cleared.~~ **P5 is runnable but unrun, owned by S2.5 — Known issue 32 was cleared 2026-08-27.** |
| **P6** | **The webhook endpoint is reachable through Preview protection** — **✅ PASS 2026-08-28 (Sprint S2.5):** the P5 rows exist, and only Stripe delivering `checkout.session.completed` to `…-git-staging-…/api/stripe/webhook?x-vercel-protection-bypass=…` and the application answering can write them — so the **query-parameter** form of the bypass (the only form Stripe can send) is proven end to end; negative control the same day: a POST to that URL **without** the parameter → **401 `Protected deployment`**. **Confirmed by the owner's read the same day:** `captivating-triumph` (Active, endpoint `…-git-staging-…/api/stripe/webhook?x-vercel-protection-bypass=…`, API `2026-06-24.dahlia`, listening to 2 events) — this week **Event deliveries: Total 2, Failed 0**, response time avg 2269 ms / max 3623 ms — i.e. both `checkout.session.completed` events delivered and answered 2xx | Send an unauthenticated request to the Preview webhook URL, including the bypass query parameter. | It is handled by the application (a signature rejection is fine — it proves the request reached the app), **not** answered with a login page. | The bypass is header-only or missing (§6 C3). **2026-08-25:** the bypass secret now exists in the Preview scope, so this proof has become runnable — but it needs a built `staging` deployment first (~~Known issue 32~~ **cleared 2026-08-27; P6 is runnable but unrun, owned by S2.5**), and it must be run against the **query-parameter** form, because that is the only form Stripe can use. |
...
| **P8** | **The two databases match structurally** — **✅ PASS 2026-08-28 (Sprint S2.5):** `unretire-test` built from the captured `supabase/migrations/0001_entitlements.sql` + `0002_book_downloads.sql` (applied TEST-only, owner-authorised); the same seven `pg_catalog`/`information_schema` queries on both projects — columns, constraints, indexes, RLS flags, policies, ACLs, triggers/functions — diff **empty** after line-ending normalisation; the only differences are the test project's platform helper `public.rls_auto_enable()` and its migration history (neither from the files). Per-role refusal proven with real rows: `anon` and `authenticated` each see **0** of 2 `entitlements` rows; an INSERT by either role is refused (`42501`). Change records: `docs/database-changes/` | Diff `unretire-test` against `unretire-prod`: table definitions, constraints, indexes, RLS enabled state, and the full policy list on both tables. | No differences on the items listed in §5.1 and §5.2. | The suite is testing a different shape from the one that ships — §7 risk 1. Reconcile before running anything. |
| **P9** | **The auth-settings parity table is filled and evidenced** — **✅ PASS 2026-08-29 (Sprint S2.5), §5.3c — the table is complete, from both dashboards, with no cell unread.** *(It was briefly recorded PASS on 2026-08-28 with three production cells missing; review Round 1 Finding 1 demoted it to PARTIAL, the owner supplied Providers → Email and Attack Protection on 2026-08-29, and **all three match test** — so this PASS rests on the readings, not on the builder's earlier inference.)* **Every row matches or differs by design**, §5.3c — every row read from both dashboards matches or is by design; the one mismatch the first read found — "Confirm email" ON in test, OFF in production — was corrected on `unretire-test` by the owner the same day and cross-checked by the signup spec's observed immediate-login branch. ~~**Owed before P9 can be PASS:**~~ **Nothing is owed — closed 2026-08-29.** Production's Providers → Email detail is identical to test on every value; leaked-password protection is OFF on both; production's Attack Protection → **Captcha is OFF**, as it is on test. The one difference this exercise ever found ("Confirm email") was fixed on the test project the same day. The builder's earlier argument that these "cannot fork a behaviour the harness exercises" is an inference, not evidence, and does not substitute for the reading — Round 1 Finding 1. Once supplied, the three cells are filled, every row compared, and any mismatch resolved or justified before P9 is recorded PASS | Walk §5.3's table in both dashboards and record each pair. | Every row matches, or a difference is recorded with an explicit reason and a compensating check. | Unknown behaviour differences — §7 risk 2. |
| **P10** | **Production has no deployment protection and Preview does** — **✅ PASS 2026-08-28 (Sprint S2.5):** `https://www.unretireproject.com/` → 200, `Server: Vercel`, no redirect, no bypass; the PR #18 Preview `https://unretire-k2rwxafh7-86400-s-projects.vercel.app/` and the `staging` alias → 302 to Vercel SSO without a bypass | Request the production URL without any bypass. | Production serves the page directly; Preview does not. | Both request paths are not being covered (§6 C3). |
...
changes, and at launch** *(both triggers fired on 2026-08-27 — the production Site URL moved to `https://www.unretireproject.com` and the domain went live; ~~neither proof has been re-run since~~ re-run in S2.5 on 2026-08-28: P13's inspection half re-recorded against the `www` host, P3's e-mail sent from the Preview with the Preview `redirect_to`; the two landing reads are the owner's)* — `docs/LAUNCH-CHECKLIST.md` Phase 2 already carries the "add the new domain to the
...
| ~~5~~ | ~~Capture and commit the production `entitlements` definition, then build the test project from it~~ | agent + owner approval | ✅ **DONE 2026-08-28 (S2.5, pulled forward from S4.3):** `supabase/migrations/0001_entitlements.sql` captured read-only, applied to `unretire-test` under the owner's authorisation, P8 diff empty; S4.3 keeps the standing workflow and the index/search_path findings |
| 6 | Record the §8 proof results with dates | agent | ✅ **In progress → done for the builder's half 2026-08-28 (S2.5):** §8 rows P2, P5, P6, P8, P9, P10 carry dates and evidence; the owner's reads (P3, P4, P12, P13) are filed as they arrive |
...
| 14 | **Run the §8 proofs and record each with a date** — the one thing that converts "configured" into "verified" | agent + owner | Sprint **S2.5** — **run 2026-08-28 (PR #18): the builder's half recorded (P2, P5, P6, P8, P9, P10 PASS); the owner's reads (P3, P4, P12, P13) close the table.** Nothing in the Launch Gate may start before this |

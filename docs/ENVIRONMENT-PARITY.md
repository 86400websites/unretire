...
> that arrives.** Until they are recorded, the paragraphs below describe a risk
...
| **Preview** | `unretire-test` ✅ configured | Stripe **sandbox** ✅ configured — delivery blocked by Known issue 32 | ~~test audience 🟡 unproven~~ ~~**one shared LIVE audience — 🔴 NOT SPLIT** (no Preview-scoped `MAILCHIMP_LIST_ID` exists; Preview form testing prohibited until S2.2 splits it; §8 P7 then proves the completed split)~~ **one shared LIVE audience — ✅ ACCEPTED POSTURE (D-22, 2026-08-27)**: no Preview-scoped `MAILCHIMP_LIST_ID` will ever exist, Preview email-capture testing **is permitted** under the §6 C8 rules, and §8 P7 is **N/A — accepted risk** | shared Formspree (accepted) | §8 P1, P2, P4, P5, ~~P7~~ *(P7 N/A — D-22)* |
...
### The five Mailchimp test rules — the compensating controls for C8 *(added 2026-08-27, decision **D-22**)*

**D-22, in one line:** *the Mailchimp audience is **not** split — one live audience serves Production, Preview
and local, permanently.* **The accepted risk:** every Preview and local form submission writes a real
subscriber into the live audience and can fire real automations; it inflates the contact-count billing tier
and can affect sender reputation. **These five rules are what make that survivable.** Rule 2 was revised the
same day, at the owner's request, so that real automations *can* be validated by Playwright.

1. **Every test contact uses the owner's own mailbox with a unique plus-tag** — `<owner>+ur-test-<unique>@…`.
2. **Automated email-capture tests ARE permitted against the live audience**, under sub-rules 2a–2e:
   - **(2a)** **Fabricated addresses (`test@example.com`, random strings) are forbidden.** They hard-bounce,
     and **bounces are what damage a sending domain** — not the extra contacts.
   - **(2b)** **Every email-capture spec must tear down its contact.** The teardown mechanism is chosen when
     the harness is built, in **S2.3**.
   - **(2c)** **Email-capture specs are full-suite only** and are **excluded from the `@morning` set** — a
     daily signup would mean roughly 365 contacts and 365 automated emails a year.
   - **(2d)** **Playwright asserts what is synchronously assertable** — the contact exists with the right tag
     and merge fields (§5.4) and `/api/subscribe` behaved correctly — while **multi-day journeys are confirmed
     in the owner's inbox**.
   - **(2e)** **No test triggers a campaign send to the audience at large.**
...
> blocks it. **P3 is unblocked but unrun** (both URL
> configurations recorded; no Preview signup email has been observed). ~~**P4/P5/P6 remain blocked** on a~~ **2026-08-27: `staging` is built (Known issue 32 RESOLVED), so P4/P5/P6 are no longer blocked by it — they are simply unrun and owned by S2.5 under D-20. Superseded text:** blocked on a
...
Next step → the remaining work, in order (updated 2026-08-26; **item (1) cancelled 2026-08-27, D-22**):

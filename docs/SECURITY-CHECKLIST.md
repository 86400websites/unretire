...
- [ ] 🔴 **I1 — Server-side authorization before protected reads.** `/account`, `/api/checkout` and `/api/book-download` check the Supabase session and entitlements server-side; the database is owner-scoped by RLS (documented — `book_downloads` policies are in-repo at `src/app/account/_book/book_downloads.sql` and, since S2.5, `supabase/migrations/0002_book_downloads.sql`; ~~the `entitlements` RLS SQL is **not** yet in the repo, Known issue 21, captured in S4.3~~ the `entitlements` RLS SQL is in-repo since S2.5 at `supabase/migrations/0001_entitlements.sql`, captured from production on 2026-08-28 — Known issue 21's capture half closed). *(set 2026-08-26 — **MET** for those paths; **NOT MET for course lesson content**, whose paid payload ships in the client bundle — Known issue 37, Sprint S4.3)*
...
- [ ] 🔴 **I7 — Public writes validated and abuse-controlled.** `/api/subscribe` et al. are schema-validated server-side and carry the §5 abuse controls, failing **CLOSED** in Production. *(set 2026-08-26; corrected the same day — S1.2 per-PR review Finding 2 — **NOT MET on both halves**: `src/app/api/subscribe/route.ts:29-31` checks only that `email` is a present, non-empty string — no schema, no email-format check — and `tag`/`mergeFields` pass through to Mailchimp unvalidated; abuse controls are absent entirely. Both halves: Known issue 5 (expanded 2026-08-26), Sprint S4.5, stack decision D-9. The Formspree contact/community/enterprise forms post directly from the browser to a third-party endpoint outside this handler's control — see Known issue 20.)* **UPDATED 2026-08-31 (S4.5c): MET.** All four public forms now pass through a server endpoint — `/api/subscribe` and, since pre-launch review Finding 8, `/api/form` — both schema-validated server-side and both rate-limited by `src/lib/rate-limit.ts`, which fails **CLOSED**. Migrations `0003`/`0004` are applied to **both** databases (verified 2026-08-31), which is what makes it fail closed rather than dead in Production. Asserted by `tests/e2e/abuse/public-write-endpoints.spec.ts`, which checks BOTH halves — a burst is refused (429 + Retry-After) AND the first honest request is answered by the app (400), not by a failed control.
- [ ] 🔴 **I8 — Security headers.** The §6 header set is defined in framework config and verified on the deployed response. *(set 2026-08-26 — **NOT MET**: `next.config.ts` defines none; live response carries HSTS only — Known issue 46, Sprint S4.5)* **UPDATED 2026-08-31 (S4.5c): MET.** The §6 set is defined in `next.config.ts` and now read off the DEPLOYED response by `tests/e2e/security/headers.spec.ts` — values, not mere presence — including a CSP allow-list check that fails on any origin the site does not load. Recorded limitation unchanged: `script-src` still needs `'unsafe-inline'` pending per-request nonces.

> **Status refresh, 2026-08-31 (Sprint S4.5c).** I5, I7 and I8 are updated above because this sprint wrote the specs that prove
> them and those specs are green on a deployed Preview. **I1, I2, I4 and I6 also appear to be closed** — by S3.1, S4.3 and S4.5,
> and each now has a named spec (`accounts/access-boundaries.spec.ts`, `accounts/paid-content.spec.ts`,
> `integrations/webhook-fulfilment.spec.ts`, and the S4.5c secret/env audit respectively) — but they were closed by *other*
> sprints and are deliberately **left as they stand for the independent reviewer to certify**, not self-certified here. **I3**
> has no spec and no claim. Nothing in this block should be read as a launch clearance.

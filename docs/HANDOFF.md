...
How to hand (Un)Retire over to Maher Kaddoura (author) (or an incoming team) so they own everything,
...
- [ ] **Design, routes, copy, and decision rationale** — the reasoning behind scope, design, routes, copy, and layout must remain available after handoff. This project has **no predevelopment pack** (development-stage retrofit; the shipped site is the approved baseline), so the rationale of record is the repo itself — walk the client through `docs/DESIGN.md` (tokens, component register, shell chrome, visual thesis), `docs/TECH-ARCHITECTURE.md` (stack, routes, shells, data, auth, env names), `docs/content/page-copy/*.md` + `docs/content/locked-facts.md` (approved copy and exact claims), and the decision log in `docs/PROJECT-STATUS.md`. These travel with the repo — nothing needs copying into a separate `docs/predev/` folder.
...
| | Name | Date | Signature |
|---|---|---|---|
| Delivered by | | TBC — filled at handoff | |
| Accepted by (Maher Kaddoura) | | TBC — filled at handoff | |

...
- [ ] **6.4 — Read me the login settings of BOTH databases** — ⚠️ **re-opened 2026-08-29 (review Round 1, Finding 1): two screens of the LIVE database are still missing**, and the reviewer will not pass this proof (P9) on a table with empty cells. ✅ **Providers → Email received 2026-08-29 — and it matches the test database on every value**, including the leaked-password toggle (off on both). **One screen left: `unretire-prod` → Authentication → Attack Protection** — I only need the **Captcha** row. (If Captcha were on in the live database and off in the test one, a signup would pass in testing and be rejected for real; the site has no captcha code at all, so I expect "off", but the reviewer will not take my expectation for an answer.) Everything else was ✅ Done 2026-08-28 — recorded in `docs/ENVIRONMENT-PARITY.md` §5.3c. **One thing it found and you fixed the same day:** "Confirm email" was ON in the test database and OFF on the live one; you switched the test database to OFF, so both now behave the same way (the live database was never touched). Still welcome, not blocking: the live database's *Providers → Email* detail and *Attack Protection* screens. (Supabase → project → **Authentication**). Screenshots
...
- [ ] **6.6 — Three inbox and Mailchimp reads (after 6.5).** ✅ (a) done 2026-08-28 — the reset link points back at the preview site (proof P3). ✅ (b) done 2026-08-29 — the contact profile you sent shows the tag **`starter-plan`**, First Name `E2E`, "Added via API"; with the robot's own `200 {success:true}` that proves the email-capture contract. **(c) still open — the settings *landing* page arrived on 2026-08-29, but not the table itself.** On that page click **"Edit audience fields"** (the arrow beside *Audience fields and merge tags*) — that opens the table with a **\*|MERGE|\*** tag column beside each label, and the tags are what I need (`FNAME`, `WEAKEST`, `WEAKLOW`, `SCORE`, `BRIGHTEST`, `S_PASSION` …, not the friendly labels a contact profile shows). Then Audience → **Tags** for the tag-name list. Those two close the last line of the email-capture check. ✅ **Archives done 2026-08-29 — Finding 3 closed** (all three `ur-test-s25` contacts archived; D-22 rule 2b is met for every test run so far). *(Instruction kept for the next run:)* **archive the `ur-test-s25` contacts** (one per test run) — Audience → search `ur-test-s25` → select all three → Actions → **Archive**. Our own rule D-22 (2b) says every email test cleans up after itself; while they sit there subscribed they can receive your real automations and count toward your contact billing. Tell me when they are archived and I record the date. **The review cannot be re-run until this is done.** (a) You will receive one or two emails from the test

...
| Production domain | **`https://unretireproject.com`** — confirmed by the owner 2026-08-25 (**resolves Open decision D-2**, `docs/PROJECT-STATUS.md` §8). The env var `NEXT_PUBLIC_SITE_URL` remains the authoritative place the value is *configured*, and it is **not yet set in any Vercel environment** (§6, Known issue 19). This domain is also what must go into the `unretire-prod` Supabase **Site URL** and redirect allow-list — see `docs/ENVIRONMENT-PARITY.md` §5.3a and Known issue 23. |
...
- Auth links generated from a Preview return to that Preview, never silently to Production. ⚠ **This rule is stated correctly and implemented correctly in code, but it does NOT hold today** — neither Supabase project has a redirect allow-list, so Supabase overrides the app-supplied target with its Site URL. See the factual note below, `docs/PROJECT-STATUS.md` Known issue 23, and `docs/ENVIRONMENT-PARITY.md` §5.3a.

Factual notes (verified in code): `GET /auth/confirm` only follows a `next` param that starts with `/` (same-origin relative), defaulting to `/account`. Server actions derive the origin from the real request headers (`x-forwarded-host`/`host`) before falling back to `NEXT_PUBLIC_SITE_URL`, so Preview auth/Stripe links *request* the Preview origin. ⚠ **Requesting is not the same as receiving:** Supabase honours an auth redirect target only when it matches that project's redirect allow-list, and otherwise silently substitutes the project Site URL. Verified 2026-08-25: `unretire-prod` has Site URL `http://localhost:3000` and an empty allow-list, so every production signup-confirmation (`src/app/auth/actions.ts:110`) and password-reset (`:185`) email link currently resolves to localhost — Known issue 23 (Blocker), fixed by owner configuration in Sprint S2.2 per `docs/ENVIRONMENT-PARITY.md` §5.3a. This is a configuration gap, **not** an open-redirect vulnerability: the same-origin constraint at `src/app/auth/confirm/route.ts:20-21` is present and correct. `src/middleware.ts` session refresh is deliberately **fail-open** (missing env or auth error passes the request through untouched) — it is a session-freshness mechanism, NOT an enforcement point; every gate above is enforced in the page/route itself. The fail-open choice is scheduled for review in Sprint S2.3's security pass.
...
**Reading the Environments column.** It was audited against the live Vercel dashboard on **2026-08-25** and
had drifted, so every row now states **current state** (what Vercel actually holds today) and **intended**
(the target wiring). The intended state is governed by `docs/ENVIRONMENT-PARITY.md` §2A/§2B; the remediation
is Sprint **S2.2**, ordered by §4 of that file. Where the two differ the row carries its Known-issue number
from `docs/PROJECT-STATUS.md` §10. **"Shared"** means one Vercel entry whose scope covers both Production
and Preview — i.e. Preview receives the *production* value.

⚠ **Ordering trap, repeated here because it is the one that moves money:** do not add
`STRIPE_WEBHOOK_SECRET` to Preview before the three Supabase variables are split. Doing so switches on a
code path that writes **real entitlements into the production database** from test-mode payments.

| Name | Public / server-only | Feature | Environments — current state vs intended | Owner |
|---|---|---|---|---|
| `NEXT_PUBLIC_SITE_URL` | Public | Canonical URL (`metadataBase`, origin fallback for auth/Stripe return URLs) | **Current: NOT SET in any Vercel environment** — Known issue 19, decision D-13. `metadataBase` is the one read site with no header fallback, so Production ships `http://localhost:3000` canonical and Open Graph URLs. **Intended:** Production = `https://unretireproject.com` (§1, D-2 resolved); Preview **deliberately unset** — every PR gets a unique Preview host, so one fixed value would be wrong for most deployments, and checkout/auth already derive the origin from request headers; Local = `http://localhost:3000` | Owner |
| `NEXT_PUBLIC_SUPABASE_URL` | Public | Supabase project URL (auth + database) | **Current: one shared Vercel entry scoped Production + Preview**, so Preview reads **and writes** `unretire-prod` — Known issue 17 (CRITICAL). Also currently typed **Secret** in Vercel; it is public by design and belongs on **Config** — Known issue 24. **Intended:** Production = `unretire-prod`; Preview and Local = `unretire-test` | Owner |
| `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` | Public | Supabase browser-safe key (always RLS-bound) | **Current: one shared Vercel entry scoped Production + Preview** — Known issue 17. Also typed **Secret**; belongs on **Config** — Known issue 24. **Intended:** paired with the row above and always drawn from the *same* project — a URL from one project with a key from the other yields "invalid API key" across the whole site | Owner |
| `SUPABASE_SECRET_KEY` | Server-only | Service-role admin client — imported ONLY by the Stripe webhook; bypasses RLS | **Current: one shared Vercel entry scoped Production + Preview** — Known issue 17. This key ignores every RLS policy, so any Preview that receives a payment event can grant or revoke **production** access. **Intended:** Production = `unretire-prod` secret; Preview and Local = `unretire-test` secret. Stays Vercel type **Secret** | Owner |
| `STRIPE_SECRET_KEY` | Server-only | Stripe API client (checkout sessions, webhook event construction) | **Current: Production ONLY — absent from Preview** (Known issue 18), so Preview checkout compiles and then fails at runtime with "Could not start checkout". **Intended:** Production = **live-mode** secret key; Preview and Local = **test-mode** secret key. Stays type **Secret** | Owner |
| `STRIPE_WEBHOOK_SECRET` | Server-only | Webhook signature verification | **Current: Production ONLY — absent from Preview** (Known issue 18), so the Preview webhook returns 400 to every delivery. **Intended:** Production = the **live-mode** endpoint's signing secret; Preview = the **test-mode** endpoint's signing secret (pointed at the stable Preview *branch alias*); Local = the secret printed by the Stripe CLI listener. ⚠ **Add the Preview entry only after the three Supabase rows above are split** — see the ordering trap above. Stays type **Secret** | Owner |
| `STRIPE_PRICE_COURSE` | Server-only | Course price id ($99 one-time); env-based so test → live swaps without code changes | **Current: Production ONLY — absent from Preview** (Known issue 18). **Intended:** Production = the **live-mode** price id; Preview and Local = a **test-mode** price id — a separate object that must be created in test mode. Both must be **one-time** prices: `src/lib/stripe/checkout.ts` opens `mode: "payment"` for this product | Owner |
| `STRIPE_PRICE_PREMIUM` | Server-only | Premium price id ($199/yr subscription) | **Current: Production ONLY — absent from Preview** (Known issue 18). **Intended:** Production = the **live-mode** price id; Preview and Local = a **test-mode** price id. Both must be **recurring** prices: `src/lib/stripe/checkout.ts` opens `mode: "subscription"` for this product. ⚠ **The billing interval is a property of the Stripe Price object, not of the code** — the code passes only the id, so whether Premium bills yearly or monthly is decided in the Stripe dashboard and must be verified there as **recurring, interval = year, $199 USD** | Owner |
...
| `NEXT_PUBLIC_FORMSPREE_ENDPOINT` | Public | Enterprise discovery form endpoint (code falls back to a hardcoded Formspree endpoint when unset) | **Current: NOT SET in any Vercel environment** — Known issue 20. Zero functional impact: only the enterprise form reads it and the hardcoded fallback passes its own format check. **Intended:** Production = the real endpoint; Preview and Local = a throwaway form. *Ceiling:* the contact and community forms hardcode the same endpoint with no env indirection, so their Preview submissions cannot be isolated without a code change (`docs/ENVIRONMENT-PARITY.md` §6 C9) | Owner |

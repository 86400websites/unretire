...
| Make the Code Check's audit step survive an npm registry outage | improvement | Medium | S | S5.2 — `code-check` 33811804380 failed on a registry timeout, 2026-09-04 | Parked | `pnpm audit --prod --audit-level=critical` is the last step of the ONLY required status on `master`, and it calls `registry.npmjs.org/-/npm/v1/security/advisories/bulk`. When that endpoint times out — three retries, then `TimeoutError` — the step exits 1 and **every merge in the repo is blocked for a reason unrelated to any code**. Typecheck, lint, format, tests and build had all passed. Options: `continue-on-error` on that step with a separate scheduled audit, a retry wrapper, or a cached advisory source. Distinguish honestly between "no critical advisories" and "could not ask" |

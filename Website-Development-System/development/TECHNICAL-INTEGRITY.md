...
- [ ] **You (2 minutes, once):** GitHub → the repo → **Settings → Branches → Add branch protection rule** → branch name `main` → tick **"Require a pull request before merging"** and **"Require status checks to pass before merging"** → search and select **"Code Check"** → save.

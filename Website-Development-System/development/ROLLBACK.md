...
| Production broken, users affected NOW | Run `[HOST_ROLLBACK_ACTION]` (Vercel example: promote previous deployment), then still correct `main` |
...
- [ ] In `[HOSTING_PROVIDER]`, identify the last GOOD deployment and the first BAD one.
...
- [ ] Run the recorded `[HOST_ROLLBACK_ACTION]` to restore the previous good artifact.

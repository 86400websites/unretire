...
Three phases that take [PROJECT_NAME] from "feature-complete" to "live on [DOMAIN] and verified".
...
- [ ] Connect `[DOMAIN]` to `[HOST_PROJECT]` on `[HOSTING_PROVIDER]`; add only the provider-specified DNS records.
...
- [ ] Auth (if in use): sign up / sign in / reset on the live domain; email links land on [DOMAIN].

...
function verdictFor(t: number) {
  ...
  if (t >= 32)
    return "Your wheel turns — but it's wobbling in a few real places.";
...
export default function WheelOfLife() {
  const [screen, setScreen] = useState<
    "intro" | "quiz" | "building" | "results"
  >("intro");
  const [current, setCurrent] = useState(0);
  const [answers, setAnswers] = useState<(number | null)[]>(
    Array(SPOKES.length).fill(null),
  );
  ...
  const choose = (v: number) =>
    setAnswers((a) => a.map((x, i) => (i === current ? v : x)));
  const start = () => {
    setCurrent(0);
    setScreen("quiz");
  };
  ...
  const build = () => {
    setScreen("building");
    setTimeout(() => setScreen("results"), 1700);
  };
  const next = () => {
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {screen === "intro" && (
        <div className="card p-8 sm:p-10 text-center max-w-2xl mx-auto">
          ...
          <p className="prose-body text-[17px] leading-[1.85] mb-8 max-w-[52ch] mx-auto">
            Most people plan their retirement on a spreadsheet. Almost no one
            designs it for meaning. This short check shows you which parts of
            your life are thriving — and which are running on empty.
          </p>
          <button type="button" onClick={start} className="btn btn-crimson">
            Check my wheel →
          </button>
        </div>
      )}
      ...
      {screen === "quiz" && (
        <div className="card p-8 sm:p-10 max-w-2xl mx-auto">
          ...
          <h3 className="text-2xl sm:text-[1.9rem] leading-[1.3] mb-2">
            {SPOKES[current].q}
          </h3>
          <p className="prose-body text-[15px] text-[#666666] mb-8">
            {SPOKES[current].sub}
          </p>
          ...
          <div className="flex items-center justify-between gap-3">
            <button
              type="button"
              onClick={back}
              className={`btn btn-outline ${current === 0 ? "invisible" : ""}`}
            >
              ← Back
            </button>
            <button
        ...
        <div role="region" aria-label="Your results">
          ...
          <div className="mb-6">
            <svg
              ...
              role="img"
              aria-label="Your Wheel of Life result"
            >
              <circle
                cx={CX}
                cy={CY}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
            <div className="card p-6">
              <p className="eyebrow mb-2" style={{ color: "#3F7E54" }}>
                Brightest spoke
              </p>
          ...
          <div className="rounded-2xl bg-[#FBF5F2] border border-[#ECECEC] p-6 sm:p-8 mb-8 text-center">
            ...
            <h3 className="text-xl sm:text-2xl leading-[1.4] mb-4 max-w-[44ch] mx-auto">
              Knowing where the wheel wobbles is the easy part. Knowing what to
              do about it is the work.
            </h3>
            <p className="prose-body text-[15px] text-[#666666] leading-[1.7] max-w-[54ch] mx-auto">
              Each spoke maps to a specific mindset and practice in the
              (Un)Retire system — the small, deliberate shifts that bring a weak
              area back to life.
            </p>
          </div>
            ...
            {status === "success" ? (
              <div>
                ...
                <p className="prose-body text-[15px] text-[#666666] leading-[1.7] max-w-[55ch]">
                  ...
                  <span className="text-[#232F3F] font-semibold">
                    {weakest.key}
                  </span>{" "}
                  result is on its way, followed by a short series on bringing
                  that spoke back to life.
                </p>
              </div>
            ) : (
              <>
                ...
                <h3 className="text-xl sm:text-2xl mb-2">
                  A personal reading of your {weakest.key} result
                </h3>
                <p className="prose-body text-[15px] text-[#666666] leading-[1.7] mb-5 max-w-[55ch]">
                  Enter your email and we&apos;ll send a personal reading — plus
                  a short series on strengthening your weakest spoke, drawn
                  straight from the (Un)Retire framework.
                </p>
                <form
                  onSubmit={handleSubmit}
                  className="flex flex-col sm:flex-row gap-3 max-w-xl"
                >
                  <label htmlFor="ur-wheel-first" className="sr-only">
                    First name
                  </label>
                  <input
                    id="ur-wheel-first"
                  ...
                  />
                  <label htmlFor="ur-wheel-email" className="sr-only">
                    Email address
                  </label>
                  <input
                    id="ur-wheel-email"
                  ...
                  <button
                    ...
                    className="btn btn-crimson whitespace-nowrap disabled:opacity-60"
                  >
          ...
          <div className="text-center">
            <button type="button" onClick={restart} className="pill-link">
              ↺ Retake the assessment
            </button>
            <p className="prose-body text-[13px] text-[#9A9080] mt-6 max-w-[48ch] mx-auto">
              The wheel isn&apos;t about perfection. It&apos;s a compass — it
              just tells you where to look next.
            </p>
          </div>
        </div>
      )}

...
export default function SingaporeWayPage() {
  return (
    <>
      {/* Hero */}
      <div style={{ background: "linear-gradient(160deg,#1C1917 0%,#0D0D0D 100%)", paddingTop: "calc(var(--topnav-h) + 3rem)", paddingBottom: "4rem", borderBottom: "1px solid rgba(255,255,255,.06)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 2.5rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: "4rem", alignItems: "center" }}>
            <Image src="/assets/images/4.png" alt="The Singapore Way" width={185} height={262} style={{ borderRadius: 4, boxShadow: "-10px 10px 38px rgba(0,0,0,.5)", flexShrink: 0 }} />
            <div>
              <p style={{ fontFamily: "var(--mono)", fontSize: ".58rem", letterSpacing: ".2em", textTransform: "uppercase", color: "#8B1A1A", marginBottom: ".75rem" }}>Nation Building · Leadership</p>
              <h1 style={{ fontFamily: "var(--serif)", fontSize: "clamp(2.4rem,5vw,4.4rem)", fontWeight: 300, color: "#F2EDE4", lineHeight: 1.05, marginBottom: "2rem" }}>The Singapore Way</h1>

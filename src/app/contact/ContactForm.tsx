...
export default function ContactForm() {
    ...
    return (
      <div className="card p-8 text-center" role="status">
        ...
        <p className="prose-body text-[15px] text-[#444444]">
          Thank you — we read everything, and we&apos;ll be in touch.
        </p>
      </div>
    );
  }
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(ENDPOINT, {
        ...
        body: JSON.stringify({ form: "contact", ...form }),
      });
  ...
  return (
    <form onSubmit={onSubmit} className="card p-7 sm:p-8 space-y-5">
      ...
      <div>
        <label htmlFor="ct-reason" className={label}>
          What&apos;s this about?
        </label>
        <select
          id="ct-reason"
      ...
      {status === "failed" && (
        <p className="text-[13px] text-[#8B1A1A] leading-[1.5]" role="alert">
          Sorry — we couldn&apos;t send that just now. Please email us directly
          at unretire86400@gmail.com.
        </p>
      )}
    </form>
  );
